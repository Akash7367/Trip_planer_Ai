...
The **Model Context Protocol (MCP)** forms the standardized communication layer between our AI Agents and external services (APIs, search engines, database queries, and utilities). Centralizing these interactions within an MCP Server isolates runtime side-effects, manages API keys in a single boundary, and standardizes data interfaces.

### 🔌 Communication Channel & Protocol

The FastAPI application server starts the MCP Server as a sub-process and communicates via a bidirectional **Stdio (Standard Input / Standard Output) Transport channel**. All message transactions use the **JSON-RPC 2.0** protocol format.

```mermaid
sequenceDiagram
    participant Agent as LangGraph Agent Node
    participant Gateway as FastAPI Tool Gateway
    participant Server as MCP Server (Stdio Pipe)
    participant Registry as Tool Registry
    participant Handler as Tool Handler Function

    Agent->>Gateway: Invokes tool request
    Gateway->>Server: Write to stdin: {"jsonrpc": "2.0", "method": "tools/call", "params": {"name": "weather_lookup", "arguments": {"location": "Goa", "days": 5}}, "id": 1}
    Server->>Registry: Lookup "weather_lookup"
    Registry-->>Server: Retrieve class & schema validator
    Server->>Server: Validate arguments against Pydantic schema
    Server->>Handler: Run weather_lookup("Goa", 5)
    Handler-->>Server: Return output dict / exception
    Server->>Gateway: Write to stdout: {"jsonrpc": "2.0", "result": {"content": [{"type": "text", "text": "..."}]}, "id": 1}
    Gateway-->>Agent: Parsed ToolResult payload
```

### 📦 End-to-End Tool Calling Example (`weather_lookup`)

Here is how the pipeline registers, validates, and runs a tool like `weather_lookup` defined under `mcp_server/tools/weather_lookup.py`:

#### 1. Define input validation model (Pydantic Schema)
Under `mcp_server/schemas/tools.py`:
```python
from pydantic import BaseModel, Field

class WeatherLookupInput(BaseModel):
    location: str = Field(description="The city or destination name to get weather for")
    days: int = Field(default=7, description="Number of forecast days (1-14)")
```

#### 2. Declare and Register the Tool
Under `mcp_server/tools/weather_lookup.py`:
```python
...
def weather_lookup(location: str, days: int = 7) -> dict:
    ...
    # Fetch data from weather API and format...
    return {"location": location, "forecast": [...]}
```

#### 3. Standard JSON-RPC Call & Response Payloads
* **Request (fastapi → mcp_server stdin):**
  ```json
  {
    "jsonrpc": "2.0",
    "method": "tools/call",
  ...
  ```
* **Response (mcp_server stdout → fastapi):**
  ```json
  {
    "jsonrpc": "2.0",
    "result": {
      "content": [
        {
          "type": "text",
          "text": "{\"location\": \"Goa\", \"forecast\": [{\"day\": 1, \"temp_c\": 22, \"condition\": \"Sunny\"}]}"
        }
      ]
    },
    "id": 42
